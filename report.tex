...
\begin{equation}
  ...
  \quad \phi' = \phi + \delta_\epsilon\phi,
\end{equation}
which follows from relabeling the variable of integration. Suppose
the field transformation preserves the path integral measure
with $\delta_\epsilon\phi= \epsilon\lambda(\phi,x)$, then
...
\end{equation}
interchanging the spacetime and path integrals we have
\begin{equation}
  \Braket{ \partial^x_\mu j^\mu(x)\prod^n_{i=1}{\mathcal{V}_i(z_i)} } = 
  -i\mathlarger{\mathlarger{\sum}}_{k=1}^n{\delta^{(4)}(x - z_k)
    \Braket{ \lambda(\phi, z_k)\frac{\partial\mathcal{V}_k}{\partial\phi}(z_k)
  \prod_{i \neq k}^n{\mathcal{V}_i(z_i)}} }.
\end{equation}
These delta function terms are called Schwinger/contact terms 
and in the case that none of the insertion points coincide we have 
