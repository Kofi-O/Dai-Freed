...
\end{equation}
Where $\approx$ denotes an on-shell equality.
...
\end{equation}
Where $E_k$ are the Euler-Lagrange derivatives and $\theta^\mu$ is the symplectic potential.
By considering the shift in the action we arrive at Noether's first theorem; an 
...
crucial for gauge theories. Noether's original formulation considers
...
\end{equation}
Under $R_\xi$ the action shifts by
\begin{equation}
...
\end{equation}
By the fundamental lemma of the calculus of variations we obtain the 
...
by the Poincaré lemma we obtain Noether's second theorem;
...
is determined upto addition by a divergence-free quantity) 
it can be uniquely determined if
one demands the superpotential obeys additional constraints (
such as asymptotic conservation). In the case of electrodynamics
...
as in (5.6) and determining the required gauge transformation
\begin{equation}
