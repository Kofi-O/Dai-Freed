...
\end{equation}
Note that $K(\vec{x}) = \int{d^3\vec{y}\,[K(\vec{y}) \delta(\vec{x} - \vec{y})]}$
...
\end{equation}
\begin{equation}
  \{\phi_n(\vec{z}), \pi_m(\vec{w}) \} = \delta_{nm}\delta(\vec{z} - \vec{w})
\end{equation}
Which are canonically quantised to the commutator and anti-commutator relations 
for bosonic and fermionic fields operators respectively acting on a Fock space
(corollary of the spin-statistics theorem)
\begin{equation}
  {[\hat{\phi}_n(\vec{z}), \hat{\phi}_m(\vec{w}) ]}_\pm = 0 = {[\hat{\pi}_n(\vec{z}), \hat{\pi}_m(\vec{w}) ]}_\pm
\end{equation}
\begin{equation}
  {[ \hat{\phi}_n(\vec{z}), \hat{\pi}_m(\vec{w}) ]}_\pm = i\hbar\delta_{nm}\delta(\vec{z} - \vec{w})
\end{equation}
