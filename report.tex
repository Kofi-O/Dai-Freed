...
Suppose we promote a classically conserved current to a field operator $\hat{\jmath}^\mu(x)$
and the operators $\{\hat{\mathcal{O}}_i(z_i)\}$ are functions of spacetime. The 
derivative of the correlator is [WE SUPPRESS THE HATS IN THE CORRELATOR
TO INDICATE A TIME-ORDERED PROD][ADD PROOF L8R]
\begin{equation}
  \partial^x_{\mu}\!\Braket{j^\mu(x)\prod^n_{i=1}{\!\mathcal{O}_i(z_i)}} = 
  \Braket{\partial^x_{\mu}j^\mu(x) \prod^n_{i=1}{\!\mathcal{O}_i(z_i)}} + 
  \mathlarger{\mathlarger{\sum}}_{k = 1}^{n}{ \delta(x_0 - z_{k_0})
    \!\Braket{\left[j^0(x), \mathcal{O}_k(z_k)\right]\prod^n_{i\neq k}{\!\mathcal{O}_i(z_i)} } }
\end{equation}
Applying the classical conservation law for the current we have
the Ward-Takahashi identity
\begin{equation}
  \partial^x_{\mu}\!\Braket{j^\mu(x)\prod^n_{i=1}{\mathcal{O}_i(z_i)}} = 0; \quad x \neq z_k
\end{equation}
