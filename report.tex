...
Given two functionals, $F$ and $G$, of the dynamical fields
\footnote{n.b. $K({x}) = \int{d{y}\,[K({y}) \delta(x - y)]}$} given by 
\begin{equation}
...
inconsistent and predicts negative energy states which require 
...
The path integral is a method of performing calculations in 
quantum field theory. It involves integrating over all 
possible field configurations, where each field configuration 
is given a weighting.

A useful technique when performing computations in quantum field theory is that of a Wick rotation.
A Wick rotation involves 
rotating the time axis by $\pi / 2$ in the complex plane, by 
multiplying time values by $i$. Under this transformation the Minkoski metric becomes the Euclidean 
metric. So one can perform a Wick rotation, compute the 
problem in Euclidean space and, 
via analytic continuation, obtain a solution to the 
original problem.

An important concept in the path integral approach is the 
partition function. This is the probability amplitude of a 
vacuum state at $t \to -\infty$ evolving back into a vacuum 
state at $t \to \infty$ in the presence of a Schwinger source 
field, $J(t)$. We denote this
\begin{align}
Z[J] \sim \langle 0 , \infty | 0 , -\infty \rangle ^{J} .
\end{align}
Using a Wick rotation one can derive the following formula
\begin{align}
Z[J] = \int \mathcal{D} \phi \exp\left[i \int^{\infty}_{-\infty}dt (L + J \phi + i \epsilon)\right] ,
\end{align}
where the $i \epsilon$ term is known as 
Feynman's $i \epsilon$ prescription [CITATION]. It 
is an infinitesimally small rotation away 
from any poles on the real line, and we 
omit it for clarity from here on.

We define the quantum action as
\begin{align}
...
the Dirac Lagrangian's axial symmetry. For the theory of massless
Dirac fermions coupling to an axial gauge field in \textcolor{red}{eq5.22}, 
